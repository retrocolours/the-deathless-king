document.addEventListener("DOMContentLoaded", () => {
    ...
    // Scenes with text and choices
    const scenes = {
        premise: {
            text: greetingText,
            choices: [
                { text: "Strength", nextScene: 'explore', imageSrc: "./assets/images/sila.png", imageAlt: "Rune of Strength" },
                { text: "Protection", nextScene: 'explore', imageSrc: "./assets/images/obereg.png", imageAlt: "Rune of Defense" }
            ]
        },
        explore: {
    ...
    function loadInventory() {
        // Loads saved inventory items on page load
        savedInventory.forEach(item => {
            const listItem = document.createElement('li');
            listItem.textContent = item;
            inventoryList.appendChild(listItem);
        });
        // Ensures the inventory box is visible if there are any items
    ...
    function startGame() {
        loadInventory(); // Ensures inventory shows at game start
        if (!gameStarted || !savedScene) {
            currentState = { scene: 'premise' };
            localStorage.setItem("gameStarted", "true");
            typeText(scenes.premise.text, () => renderChoices(scenes.premise.choices));
        } else {
    ...
    function renderChoices(choices) {
        choiceButtons.innerHTML = ''; // Clears previous choices
        choices.forEach(choice => {
            const button = document.createElement('button');
            button.classList.add('game-container__choice-button');
            button.innerText = choice.text;

            // Adds image if available for the choice
            if (choice.imageSrc) {
    ...
    function updateInventory(choice) {
        ...
        if (item && !savedInventory.includes(item)) { 
            ...
            // Adds item to inventory array and save it in localStorage
